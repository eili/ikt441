...
class City:
    def __init__(self, line):
        ...
        self.populationTxt = self.getPart(line, 4)
    ...
    def hasPopulation(self):
        return len(self.populationTxt)>1
...
def plotNodes(nodes):
    ...
    for node in nodes:
        city = node.city
        plt.plot(city.longitude, city.latitude, "ro", color="blue")
...
def plotEdges(edges, includeCity):

    for edge in edges:
        x = []
        y = []
        c1 = edge.fromNode.city
        c2 = edge.toNode.city
        if includeCity:
            plt.plot(c1.longitude, c1.latitude, "ro")
            plt.plot(c2.longitude, c2.latitude, "ro")
        x.append(c1.longitude)
        x.append(c2.longitude)
        y.append(c1.latitude)
        y.append(c2.latitude)
        plt.plot(x, y)
...
maxNodeDist=totalDist / 4

edges = getEdges(nodes, maxNodeDist, startCity)
for e in edges:
    print(e)

#plotEdges(edges, False)
